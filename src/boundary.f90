module boundary
    use setup, only: N, x, b, boundary_type, &
                     ...
                     hloc, pi, fourpi, rho0_sphere, &
                     offset_x1, offset_x2, rsp1, &
                     rho1, rsp2, rho2, semi_x, semi_z
...
contains
    ...
    DOUBLE PRECISION function boundary_value(i, k, l)
        ...
        double precision :: prom1, prom2, lambda, I1
        ...
        case (0) !homogenous sphere
            if (radius <= rsp_sphere) then
                boundary_value = -2*pi*G*rho0_sphere*(rsp_sphere**2 - radius**2/3.d0)

            else
                boundary_value = -4.d0*pi*G*rho0_sphere*rsp_sphere**3/(3.d0*radius)
            end if

        case (1) !binary
            if (radius1 <= rsp1) then
                grav1 = -2*pi*G*rhosp1*(rsp1**2 - radius1**2/3.d0)
            else
                grav1 = -4*pi*G*rhosp1*rsp1**3/(3.d0*radius1)
            end if
            if (radius2 <= rsp2) then
                grav2 = -2*pi*G*rhosp2*(rsp2**2 - radius2**2/3.d0)
            else
                grav2 = -4*pi*G*rhosp2*rsp2**3/(3.d0*radius2)
            end if
