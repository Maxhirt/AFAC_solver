module output
    use setup, only: N, relative_error, x, ana_solution
    ...
    subroutine output_results_global(run)
        ...
        do img = 1, num_images()
            if (this_image() == img) then
                ...
                write (u_comb, '(I5, 2X, 6(E14.7, 2X), I2)') xpos, ypos, zpos, x(i, k, l), ana_solution(i, k, l), rel_error(i, k, l)
